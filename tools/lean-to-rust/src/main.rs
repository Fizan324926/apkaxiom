...
fn render(ext: &Extracted) -> Result<String, ExtractError> {
    ...
    let fn_sig = if ext.arg.is_empty() {
        format!("pub const fn {}() -> u64 {{ {} }}", ext.name, body_rust)
    } else {
        let name = &ext.name;
        let arg = &ext.arg;
        format!("pub const fn {name}({arg}: u64) -> u64 {{ {body_rust} }}")
    };
    ...
    let tests = if ext.test_cases.is_empty() {
        ...
        let cases = ext
            ...
            .map(|tc| {
                format!(
                    "    #[test] fn {test_name}() {{ assert_eq!({fn_name}({arg}), {exp}); }}",
                    test_name = tc.name,
                    fn_name = ext.name,
                    arg = tc.arg_value,
                    exp = tc.expected,
...
mod tests {
    ...
    fn nullary_def() {
        ...
        assert!(r.contains("pub const fn answer() -> u64 { 42 }"));
    }
}
