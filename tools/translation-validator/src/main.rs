...
fn diff_outputs(lean: &str, rust: &str, max_show: usize) -> Result<usize, ValidatorError> {
...
fn run() -> Result<(), ValidatorError> {
    ...
    // Materialise hex inputs to a temp file (so we can pipe the
    // same file into both evaluators by FD).
    ...
    // Both evaluators emit one line per *non-empty* hex line (an
    // empty input is uniformly skipped on both sides — see
    // Apkaxiom.Tv.LfhEval.main and `lfh-eval-rust::main`). The
    // diff therefore covers `non_empty_inputs` lines.
    let non_empty = items.iter().filter(|(_, v)| !v.is_empty()).count();
    let agreed = diff_outputs(&lean_out, &rust_out, args.max_show)?;
    ...
    println!(
        "AGREE: {agreed}/{non_empty} non-empty inputs Lean ↔ Rust byte-identical{skipped_note}."
    );
    if agreed != non_empty {
        return Err(ValidatorError::LineCountMismatch {
            lean_lines: agreed,
    ...
    if let Some(out) = &args.receipt_out {
        // The receipt is a single line:
        // <iso-timestamp> <corpus-sha256> <lean-output-sha256>
        // <rust-output-sha256> <total>/<total>.
        // (Both output hashes equal at this point.)
        let lean_hash = sha256(lean_out.as_bytes());
        let rust_hash = sha256(rust_out.as_bytes());
        // Deterministic: no timestamps, no elapsed times.
        // `make tv-check-receipt` verifies byte-identical
        // reproducibility, so the receipt must contain only
        // content-determined fields.
        let receipt = format!(
            "tv-receipt v1\ncorpus-sha256: {}\ncorpus-size: {total} (non-empty: {non_empty}, skipped: {skipped})\nlean-output-sha256: {}\nrust-output-sha256: {}\nagree: {agreed}/{non_empty}\n",
            hex_encode(&corpus_hash),
