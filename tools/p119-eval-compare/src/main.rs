...
//! `p119-eval-compare` — Timed evaluation harness for comparison benchmarks.
//!
//! Runs the full Phase-1 pipeline (ZIP parse → IR emit → signature verify →
//! BLAKE3 file digest) on every APK in a corpus directory and emits
//! comparison-ready NDJSON that includes per-APK elapsed time alongside the
//! deterministic fields.
//!
//! ## NDJSON record format
//!
//! ```json
//! {"file":"foo.apk","verdict":"accept","elapsed_ms":4.5,"ir_sha256":"…","file_blake3":"…"}
//! ```
//!
//! With `--bench`, prints a summary table (p50/p95/p99/throughput) to stdout
//! suitable for pasting into comparison documents.
...
fn main() {
    ...
    let corpus_dir = parse_flag(&args, "--corpus").unwrap_or_else(|| {
        eprintln!("Usage: p119-eval-compare --corpus <dir> [--json-out <file>] [--bench]");
        std::process::exit(1);
    });
    ...
    for apk_path in &apks {
        ...
        let fname = apk_path.file_name().unwrap_or_default().to_string_lossy();
        records.push(format!(
            r#"{{"file":"{fname}","verdict":"{verdict_str}","elapsed_ms":{elapsed_ms:.3},"ir_sha256":"{ir_sha256}","file_blake3":"{blake3_hex}"}}"#
        ));
    }
    ...
    if bench_mode {
        let coverage_pct = if n_total > 0 {
            (n_accept + n_reject) as f64 / n_total as f64 * 100.0
        } else {
            0.0
        };
        ...
        println!();
        println!("┌─────────────────────────────────────────────────────────────────────────┐");
        println!("│  APKAXIOM Bench Summary                                                 │");
        println!("├────────────────┬──────────┬──────────┬──────────┬────────────┬──────────┤");
        println!("│  Corpus        │   p50    │   p95    │   p99    │ Throughput │ Coverage │");
        println!("├────────────────┼──────────┼──────────┼──────────┼────────────┼──────────┤");
        println!(
            "│  APKAXIOM      │ {p50:>6.1}ms │ {p95:>6.1}ms │ {p99:>6.1}ms │ {throughput:>7.0} A/s │ {coverage_pct:>7.2}% │"
        );
        println!("└────────────────┴──────────┴──────────┴──────────┴────────────┴──────────┘");

        if coverage_pct < 99.0 {
            eprintln!("FAIL coverage gate: {coverage_pct:.2}% < 99%");
            std::process::exit(1);
        }
        println!("PASS coverage gate: {coverage_pct:.2}% ≥ 99%");
    }
