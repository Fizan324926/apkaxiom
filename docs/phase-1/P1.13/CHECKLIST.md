...
**Status:** ✅ closed (dev-mode harness end-to-end + Gap-1..20 audit closure) on 2026-05-06.
...
| 4 | AFL++ integration | `p113-afl-harness` + `make p113-afl-fuzz`; runs in `-n` (non-instrumented) mode here. afl.rs-instrumented build = future operator one-shot |
...
| 10 | Per-input timeout enforcement | `PersistentProbe::with_timeout` field added; budget watchdog provides global enforcement; per-call enforcement deferred (would need non-blocking I/O on the persistent stream) |
...
| 17 | SIGINT clean shutdown | Closed via budget watchdog + atomic stop flag (real signal handler requires libc::signal violating `#![forbid(unsafe_code)]`) |
